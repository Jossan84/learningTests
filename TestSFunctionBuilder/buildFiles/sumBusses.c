 ...
 *
  ...
  *  ------------------------------------------------------------------------- 
* Created: Thu Jan 30 10:02:32 2020
*/
#define S_FUNCTION_LEVEL 2
#define S_FUNCTION_NAME sumBusses
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
/* %%%-SFUNWIZ_defines_Changes_BEGIN --- EDIT HERE TO _END */
#define NUM_INPUTS          2
/* Input Port  0 */
#define IN_PORT_0_NAME      bus1
...
extern void sumBusses_Outputs_wrapper(const Bus1 *bus1,
			const Bus2 *bus2,
			real_T *result);
...
static void mdlInitializeSizes(SimStruct *S)
{
  ...
  /* Register Bus1 datatype for Input port 0 */

    #if defined(MATLAB_MEX_FILE)
    if (ssGetSimMode(S) != SS_SIMMODE_SIZES_CALL_ONLY)
    {
      DTypeId dataTypeIdReg;
      ssRegisterTypeFromNamedObject(S, "Bus1", &dataTypeIdReg);
  ...
  /* Register Bus2 datatype for Input port 1 */

    #if defined(MATLAB_MEX_FILE)
    if (ssGetSimMode(S) != SS_SIMMODE_SIZES_CALL_ONLY)
    {
      DTypeId dataTypeIdReg;
      ssRegisterTypeFromNamedObject(S, "Bus2", &dataTypeIdReg);
      if(dataTypeIdReg == INVALID_DTYPE_ID) return;
...
static void mdlStart(SimStruct *S)
{
	...
	DTypeId Bus1Id = ssGetDataTypeId(S, "Bus1");
	DTypeId Bus2Id = ssGetDataTypeId(S, "Bus2");

	int_T *busInfo = (int_T *)malloc(10*sizeof(int_T));
	if(busInfo==NULL) {
	...
	busInfo[0] = dtaGetDataTypeElementOffset(dta, bpath,Bus1Id,0);
	busInfo[1] = dtaGetDataTypeSize(dta, bpath, ssGetDataTypeId(S, "double"));
	busInfo[2] = dtaGetDataTypeElementOffset(dta, bpath,Bus1Id,1);
	busInfo[3] = dtaGetDataTypeSize(dta, bpath, ssGetDataTypeId(S, "double"));
	busInfo[4] = dtaGetDataTypeElementOffset(dta, bpath,Bus2Id,0);
	busInfo[5] = dtaGetDataTypeSize(dta, bpath, ssGetDataTypeId(S, "double"));
	busInfo[6] = dtaGetDataTypeElementOffset(dta, bpath,Bus2Id,1);
	busInfo[7] = dtaGetDataTypeSize(dta, bpath, ssGetDataTypeId(S, "double"));
	busInfo[8] = dtaGetDataTypeElementOffset(dta, bpath,Bus2Id,2);
...
{
    const char *bus1 = (const char *) ssGetInputPortSignal(S,0);
    const char *bus2 = (const char *) ssGetInputPortSignal(S,1);
    real_T        *result  = (real_T *)ssGetOutputPortRealSignal(S,0);
	...
	(void) memcpy(&_u0BUS.num1,bus1 + busInfo[0], busInfo[1]);
	(void) memcpy(&_u0BUS.num2,bus1 + busInfo[2], busInfo[3]);
	(void) memcpy(&_u1BUS.num1,bus2 + busInfo[4], busInfo[5]);
	(void) memcpy(&_u1BUS.num2,bus2 + busInfo[6], busInfo[7]);
	(void) memcpy(&_u1BUS.num3,bus2 + busInfo[8], busInfo[9]);
	
	sumBusses_Outputs_wrapper(&_u0BUS,&_u1BUS, result);
